...
public class TelaGerenciarCardapioController {
    ...
    public void initialize() {
        ...
        if (restaurante == null) {
            mostrarAlerta("Erro", "Restaurante não identificado.");
            return;
        }
        carregarCardapio();
    }
    ...
    private void carregarCardapio() {
        ...
        for (Produto p : restaurante.getCardapio()) {
            containerProdutos.getChildren().add(criarCardProduto(p));
        }
    }

    private VBox criarCardProduto(Produto p) {
        VBox card = new VBox(8);
        // Estilo visual baseado na disponibilidade (Cinza se indisponível)
        ...
        btnStatus.setStyle("-fx-background-color: " + (p.isDisponivel() ? "#f39c12" : "#27ae60") + "; -fx-text-fill: white; -fx-font-weight: bold; -fx-cursor: hand; -fx-background-radius: 5;");
        ...
        btnRemover.setStyle("-fx-background-color: transparent; -fx-text-fill: #c0392b; -fx-border-color: #c0392b; -fx-border-radius: 5; -fx-cursor: hand;");
    ...
    private void adicionarProduto() {
        ...
        dialog.setResultConverter(btn -> {
            if (btn == ButtonType.OK) {
                try {
                    ...
                    if (nome.isEmpty() || tipo == null) return null;

                    // Cria o objeto correto baseado no tipo
                    if (tipo.equals("Comida")) return new Comida(nome, desc, preco, false);
                    if (tipo.equals("Bebida")) return new Bebida(nome, desc, preco, 350); // Padrão 350ml
                    if (tipo.equals("Sobremesa")) return new Sobremesa(nome, desc, preco);
                    if (tipo.equals("Adicional")) return new Adicional(nome, preco);
                } catch (Exception e) {
                    return null;
                }
            }
        ...
        if (result.isPresent()) {
            ...
            RepositorioRestaurantes.getInstance().salvarDados(); // Persiste
            carregarCardapio(); // Atualiza tela
            mostrarAlerta("Sucesso", "Produto adicionado!");
        } else {
            // Se falhou (ex: erro de conversão ou cancelado)
            if (txtNome.getText().isEmpty() && !dialog.getResult().equals(ButtonType.CANCEL)) {
                 // Ignora se foi cancelado, avisa se foi erro
            }
        }
    }
